...
export default function NoficiationEncourageEnable() {
  ...
  const { user, userSettings } = useSelector((g: ReduxRootState) => g.ClientSocket);
  const { notificationsAllowed } = useSelector((g: ReduxRootState) => g.NotificationManager)
  ...
  useEffect(() => {
    ...
    // if notifications are already allowed, do nothing
  ...
  const encourageEnableNotifications = () => {
    dispatch(addDialog({
      ...
      subtitle: "Enable notifications to stay updated on new messages and mentorship requests.",
      buttons: [
        {
          ...
          onClick: async () => {
            ...
            if (permission === "granted") {
              dispatch(addDialogImmediate({
                ...
                subtitle: "You will now receive notifications for new messages and mentorship requests.",
              }));
            } else {
              // show user how to enable notifications from browser settings, for their specific browser
              console.log('Notification permission denied, showing instructions to enable manually');
              dispatch(addDialogImmediate({
                title: "Enable Notifications on XYZ Browser",
                subtitle: "This is how you enable notifications on XYZ Browser: ...",
              }));
            }
