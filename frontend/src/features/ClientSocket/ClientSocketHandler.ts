...
// and it has a custom event handler to handle events passed from ClientSocket.
...
function ClientSocketEventHandler(
  ...
  if (event == "disconnect") {
    ...
    // app requires socket to be authed (either with or without user) to be considered ready.
    if (payload == "authed_nouser" || payload == "authed_user") {
...
//   user: UserObj = {};
//   mentorshipRequests: ObjectAny[] = []; // unused
...
//   updateProfile(params: Partial<UserObj>, callback?: Function) {
